...
#ifndef SYSINFO_ARCH
#define SYSINFO_ARCH        "ARM-M4"
#endif
#ifndef SYSINFO_MCU
#define SYSINFO_MCU         "HC32F460PETB-LQFP100"
#endif
#ifndef SYSINFO_DEVICE_NAME
#define SYSINFO_DEVICE_NAME "HC32F460-STK"
#endif
...
#define SYSINFO_KERNEL_VERSION "AliOS-Things-rel_1.3.3"
#endif

//#define krhino_task_del(a)    

/* dynamic memory alloc test */
#if (RHINO_CONFIG_MM_TLF > 0)
#define TEST_CONFIG_MM_ENABLED                  (0)
#if (TEST_CONFIG_MM_ENABLED > 0)
#define TEST_CONFIG_MALLOC_MAX_SIZE             (16)
#define TEST_CONFIG_MALLOC_FREE_TIMES           (10)
#endif
#endif
...
#define TEST_CONFIG_STACK_SIZE                  (32)
#endif
#ifndef TEST_CONFIG_STACK_SIZE_BIG
#define TEST_CONFIG_STACK_SIZE_BIG              (64)
#endif
#define TEST_CONFIG_MAX_TASK_COUNT              (4)
...
#ifndef TEST_CONFIG_STACK_SIZE
#define TEST_CONFIG_STACK_SIZE                  (16)
#endif
#define TEST_CONFIG_SYNC_TIMES                  (10)
#define TEST_CONFIG_QUEUE_BUF_SIZE              (8)
#endif
...
static int g_var0 = 0;
static int g_var1 = 0;
static cpu_stack_t stack_buf[TEST_CONFIG_MAX_TASK_COUNT / 2][TEST_CONFIG_STACK_SIZE];
static cpu_stack_t stack_buf_big[TEST_CONFIG_MAX_TASK_COUNT / 2][TEST_CONFIG_STACK_SIZE_BIG];
...
CASE(test_task, aos_1_006)
{
    ...
    ret = krhino_task_create(&g_task[0], "task0", NULL, 2, 50,
...
CASE(test_task, aos_1_011)
{
    ...
    for (i=0; i<TEST_CONFIG_MAX_TASK_COUNT; i++) {
        printf("task%d", i);
        if(i % 2){
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                 stack_buf[(i - 1)/2], TEST_CONFIG_STACK_SIZE, task0, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
            printf("create %s OK\r\n", task_name);
        }
        else{
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                 stack_buf_big[i / 2], TEST_CONFIG_STACK_SIZE_BIG, task0, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
            printf("create %s OK\r\n", task_name);
        }
...
CASE(test_task, aos_1_012)
{
    ...
    for(i=1; i<=TEST_CONFIG_CREATE_TASK_TIMES; i++) {
        g_var0 = 0;
        printf("task%d\r\n", i);
        ret = krhino_task_create(&g_task[0], task_name, NULL, 2, 50,
                                 stack_buf[0], TEST_CONFIG_STACK_SIZE, task0, 1);
        ASSERT_EQ(ret, RHINO_SUCCESS);
        krhino_task_sleep(10);
        ASSERT_EQ(g_var0, 1);
...
CASE(test_task_comm, aos_1_014)
{
    ...
    for (i = 0; i < task_count; i++) {
        if(i % 2) {
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                     stack_buf[(i - 1) / 2], TEST_CONFIG_STACK_SIZE, task1, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
        }
        else {
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                     stack_buf_big[i / 2], TEST_CONFIG_STACK_SIZE_BIG, task2, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
        }
        krhino_task_sleep(1);
    }
...
CASE(test_task_comm, aos_1_016)
{
    ...
    for(i=0; i<task_count; i++) {
        ...
        if(i % 2) {
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                     stack_buf[(i - 1) /2], TEST_CONFIG_STACK_SIZE, task3, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
        }
        else {
            ret = krhino_task_create(&g_task[i], task_name, NULL, 2, 50,
                                     stack_buf_big[i / 2], TEST_CONFIG_STACK_SIZE_BIG, task4, 1);
            ASSERT_EQ(ret, RHINO_SUCCESS);
        }
        krhino_task_sleep(1);
    }
...
CASE(test_task_comm, aos_1_018)
{
    ...
    ret = krhino_task_create(&g_task[0], "task5", NULL, 2, 50,
                             stack_buf_big[0], TEST_CONFIG_STACK_SIZE_BIG, task5, 1);
    ASSERT_EQ(ret, RHINO_SUCCESS);

    ret = krhino_task_create(&g_task[1], "task6", NULL, 2, 50,
                             stack_buf_big[1], TEST_CONFIG_STACK_SIZE_BIG, task6, 1);
    ASSERT_EQ(ret, RHINO_SUCCESS);

//    size_t stack_free_size;
    ...
    for(i=1; i<=TEST_CONFIG_SYNC_TIMES; i++) {
        ...
        //cut_get_stack_min_free(&g_task[0]);
        //cut_get_stack_min_free(&g_task[1]);
...
#define TEST_TASK_STACKSIZE    (168)
...
{    
    sdk_init();
    krhino_task_create(&test_task_obj, "test_task", 0, RHINO_CONFIG_PRI_MAX - 2, 50,
